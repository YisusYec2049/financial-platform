...
const TRIGGER_URL = "https://srv1778161.tail6b87a9.ts.net/trigger/cruce";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!res.ok) {
      return NextResponse.json({ error: json?.error || "No se pudo iniciar la actualización" }, { status: res.status });
    }

    logAudit({
      user_email: user!.email ?? "unknown",
      action: "trigger_cruce",
    });
