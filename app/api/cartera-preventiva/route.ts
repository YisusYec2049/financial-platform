...
export async function GET(req: NextRequest) {
  ...
  const aplicarFiltros = (columnas: string, exact: boolean) => {
    ...
    // "Con pago sin aplicar" (spec 2026-08-21): las cuotas que el pipeline dejó
    // cortas por menos del umbral y avisó, porque su plata espera a que alguien la
    // asocie a mano. Es trabajo del día y nadie las va a encontrar buscando por
    // documento: quien revisa no sabe que existen.
    // Va aparte del desplegable de estado a propósito — esas 3 opciones parten la
    // cartera sin solapes (pendiente + resuelta + cerrada = total) y una cuarta que
    // cruza las tres rompería esa cuenta.
  ...
  logAudit({
    ...
    action: "query",
    filters: { search, estado, vencFrom, vencTo, pagoParcial, medioPago, payFrom, payTo, cruceFrom, cruceTo, conNotificacion, wompiTipo, multiCuota, pagoSinAplicar, diferencia, page, view: "cartera_preventiva" },
